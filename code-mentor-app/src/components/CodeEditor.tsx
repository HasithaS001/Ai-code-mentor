...
import { textToSpeech, stopAudio, getCurrentAudioElement, isGeneratingAudio, getAudioFromCache } from '../utils/tts';
...
export default function CodeEditor({ file }: CodeEditorProps) {
  ...
  const [isUsingCachedExplanation, setIsUsingCachedExplanation] = useState<boolean>(false);
  ...
  const saveExplanationToCache = (code: string, language: string, explanation: string): void => {
    try {
  ...
  const getExplanationFromCache = (code: string, language: string): string | null => {
    try {
  ...
  const clearAllCaches = () => {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.startsWith('code_explanation_') || key.startsWith('elevenlabs_audio_cache_'))) {
  ...
  const handleExplain = async () => {
    ...
    try {
      ...
      const cachedExplanation = getExplanationFromCache(selectedCode, getLanguage());
      ...
      if (cachedExplanation) {
        ...
        if (settings.language !== 'en') {
          try {
            const translated = await translateText(cachedExplanation, settings.language);
      ...
      try {
        ...
        const explanationText = explanation || (isUsingCachedExplanation ? cachedExplanation : '') || '';
        
        // Check if we have cached audio for this explanation
        const translatedText = settings.language !== 'en' ? (translatedExplanation || '') : explanationText;
        const cachedAudio = getAudioFromCache(translatedText, settings.language);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div 
        ...
        style={{ height: explanation ? `calc(100% - ${panelHeight}px)` : '100%', minHeight: 0 }}
      >
      <Editor
        height="100%"
      ...
      {explanation && (
        ...
        >
          ...
          <div className="overflow-auto flex-1 px-4">
            ...
            <div className={`flex justify-center items-center ${isPanelExpanded ? 'mb-3' : 'mb-6'} px-4`}>
              <div className="flex items-center justify-between p-1 bg-gray-100 rounded-full relative">
                {/* No sliding background - using border instead */}
                
                {/* Voice Tab */}
                <button
                  className={`px-4 py-1.5 rounded-full z-10 flex items-center justify-center transition-colors duration-300 relative ${activeTab === 'voice' ? 'text-blue-600 font-medium border-2 border-blue-500' : 'text-gray-700 hover:text-gray-900'}`}
                ...
                {/* Text Tab */}
                <button
                  className={`px-4 py-1.5 rounded-full z-10 flex items-center justify-center transition-colors duration-300 relative ${activeTab === 'text' ? 'text-blue-600 font-medium border-2 border-blue-500' : 'text-gray-700 hover:text-gray-900'}`}
                  style={{ width: 'calc(33.33% - 4px)' }}
                  onClick={() => {
                ...
                {/* Visual Tab */}
                <button
                  className={`px-4 py-1.5 rounded-full z-10 flex items-center justify-center transition-colors duration-300 relative ${activeTab === 'visual' ? 'text-blue-600 font-medium border-2 border-blue-500' : 'text-gray-700 hover:text-gray-900'}`}
                  style={{ width: 'calc(33.33% - 4px)' }}
                  onClick={() => {
            ...
            {activeTab === 'text' ? (
              <div className="prose prose-sm max-w-none px-4 scrollable-section">
              ...
              <div 
                className={`h-full scrollable-section ${isPanelExpanded ? 'px-2' : 'px-4'}`} 
                style={{ 
                  minHeight: '200px',
              ...
              >
                {isGeneratingVisual ? (
                  ...
                  <VisualExplanation 
                    code={selectedCode} 
                    language={currentLanguage || getLanguage()} 
                    panelHeight={isPanelExpanded ? panelHeight - 40 : panelHeight - 100} 
                  />
                )}
              </div>
            ) : (
                ...
                <div className="flex flex-col items-center justify-center w-full max-w-md px-4">
                  ...
                  <div className="flex items-center justify-center space-x-8 mb-2">
                    ...
                    <button
                      onClick={() => {
                        if (isPlaying) {
                          ...
                          // Start new speech or resume
                          const currentAudio = getCurrentAudioElement();
                          if (currentAudio && currentAudio.paused && currentAudio.duration > 0 && currentAudio.currentTime > 0) {
                            // Resume paused audio
                            currentAudio.play();
                            setIsPlaying(true);
                          } else {
                            ...
                            textToSpeech(explanation, settings.language, {
                              onProgress: (progress: number) => {
                                ...
                                // Once we start getting progress, we're no longer loading
                    ...
                    >
                      ...
                      <div className={isLoadingAudio ? 'opacity-0' : 'opacity-100'}>
                        {isPlaying ? (
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12" viewBox="0 0 24 24" fill="#1f61fd">
                    ...
                    <button
                          ...
                          textToSpeech(explanation, settings.language, {
                            ...
                            onComplete: () => {
                              ...
                              setTimeout(() => {
                                setIsPlaying(false);
                                setSpeechProgress(0);
                              }, 1000); // Increased delay to ensure progress bar is visible at 100%
                            },
                            playbackSpeed: playbackSpeed
                          }).catch(err => {
                            console.error('Error playing speech:', err);
                      ...
                      disabled={isLoadingAudio || !explanation}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" viewBox="0 0 24 24" fill="#1f61fd">
                    ...
                    <button
                      ...
                      className="focus:outline-none"
                      disabled={isLoadingAudio || (!isPlaying && speechProgress === 0)}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" viewBox="0 0 24 24" fill="#1f61fd">
                  ...
                  <div className="relative">
                    <button 
                      className="p-2 rounded-full text-white bg-[#1f61fd] hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed"
                    ...
                    {showSpeedMenu && (
                      <div className="absolute bottom-full mb-2 bg-white shadow-lg rounded-md p-1 z-10">
                        {[0.5, 0.75, 1, 1.25, 1.5, 1.75, 2].map((speed) => (
                          <button
                            key={speed}
                            className={`block w-full text-left px-3 py-1 text-sm ${playbackSpeed === speed ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
                            onClick={() => {
                              ...
                              // Update playback speed for future playbacks
                              // Note: We're using the local state only since the TTS utility doesn't export setPlaybackSpeed
                            }}
                          >
                            {speed}x
                          </button>
